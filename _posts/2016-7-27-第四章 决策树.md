...
给定数据集D和属性a，令$$\hat{D}$$表示D属性a上没有缺失值的样本子集，假定a有V个可取值$$\{a_1,a_2,...a_V\}$$，令$$\hat{D}^{v}$$表示$$\hat{D}$$中在属性a上取值为$$a_v$$的样本子集，$$\hat{D}_k$$表示$$\hat{D}$$中属于第$$k(k=1,2,3,…,|y|)$$类的样本子集，假定我们为每一个样本$$x$$赋予一个权重$$w_x$$
